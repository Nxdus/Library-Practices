<?php
    ...
    if (isset($_POST['create-submit']) && !empty($_FILES['image']['name'])) {
        ...
        $data = $conn->query("SELECT * FROM categories WHERE category = '$category' LIMIT 1");
        $categoryCheck =  $conn->fetch($data);

        if ($categoryCheck["category"] == $category) {
            echo "<script>alert('Category already exists');</script>";
        } else {
            if (in_array($fileType, $allowTypes)) {
                if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFilePath)) {
                    $result = $conn->query("INSERT INTO categories(category, description, image) VALUES ('$category','$description', '$filename')");
                }
            }
            ...
            <div class="register-box">
                ...
                <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST" enctype="multipart/form-data">
                        ...
                        <label for="description">
                            ...
                            <textarea id="description" type="text" name="description" placeholder="Description" required></textarea>
                        </label>
                        <br>
                        <label for="image">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width: 32px; padding: 12px;">
